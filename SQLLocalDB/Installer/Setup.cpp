/*
** SQLLocalDBView Setup - Standalone Installer/Uninstaller
** Copyright (c) 1999-2026 Tootega Pesquisa e Inovacao. MIT License.
**
** Self-contained installer that:
** - Detects if SQLLocalDBView is already installed
** - Offers to Install (if not installed) or Uninstall (if installed)
** - Extracts embedded DLL to Program Files\Tootega\SQLLocalDBView
...
// Product information
#define PRODUCT_NAME        L"SQLLocalDBView"
#define PRODUCT_VERSION     L"1.0.0"
#define PRODUCT_PUBLISHER   L"Tootega Pesquisa e Inovacao"
#define DLL_NAME            L"SQLLocalDBView.dll"
#define INSTALL_SUBDIR      L"Tootega\\SQLLocalDBView"

// Registry paths for uninstall information
#define UNINSTALL_KEY       L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\SQLLocalDBView"
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
    ...
    if (forceInstall) {
        ...
        if (isInstalled) {
            ...
            if (result == IDYES) doUninstall = true;
        }
        else {
            int result = ShowInstallDialog();
            if (result == IDYES) doInstall = true;
        }
    }
    ...
    if (doInstall) {
        ...
        std::wstring setupTarget = installDir + L"\\SQLLocalDBViewSetup.exe";
        ...
        success = true;
        if (!silentMode) ShowSuccessMessage(true);
    }
    else if (doUninstall) {
        std::wstring installDir = GetInstallPath();
        std::wstring targetDll = installDir + L"\\" + DLL_NAME;
        ...
        success = true;
        if (!silentMode) ShowSuccessMessage(false);
    }
...
bool RemoveInstallDir(const std::wstring& installDir)
{
    ...
    std::wstring pdbPath = installDir + L"\\SQLLocalDBView.pdb";
    DeleteFileW(pdbPath.c_str());

    std::wstring setupPath = installDir + L"\\SQLLocalDBViewSetup.exe";
    ...
    if (SUCCEEDED(SHGetFolderPathW(nullptr, CSIDL_PROGRAM_FILES, nullptr, 0, programFiles))) {
        ...
        RemoveDirectoryW(tootegaDir.c_str());
    }
...
int ShowInstallDialog()
{
    return MessageBoxW(nullptr,
        L"SQLLocalDBView is not installed.\n\n"
        L"Do you want to install SQLLocalDBView?\n\n"
        L"This will add support for browsing SQL Server LocalDB\n"
        L"databases (.mdf) directly in Windows Explorer.",
        L"SQLLocalDBView Setup",
...
{
    return MessageBoxW(nullptr,
        L"SQLLocalDBView is already installed.\n\n"
        L"Do you want to uninstall SQLLocalDBView?\n\n"
...
void ShowSuccessMessage(bool isInstall)
{
    if (isInstall) {
        MessageBoxW(nullptr,
            L"SQLLocalDBView has been installed successfully!\n\n"
            L"You can now browse SQL Server LocalDB databases (.mdf)\n"
            L"directly in Windows Explorer.\n\n"
            L"Right-click on an .mdf file to see additional options.",
            L"SQLLocalDBView Setup",
            MB_OK | MB_ICONINFORMATION);
    }
    else {
        MessageBoxW(nullptr,
            L"SQLLocalDBView has been uninstalled successfully.\n\n"
            L"Thank you for using SQLLocalDBView!",
            L"SQLLocalDBView Setup",
            MB_OK | MB_ICONINFORMATION);
    }
}
...
void ShowErrorMessage(const std::wstring& message)
{
    MessageBoxW(nullptr, message.c_str(), L"SQLLocalDBView Setup - Error", MB_OK | MB_ICONERROR);
}
