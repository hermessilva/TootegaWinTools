<#
.SYNOPSIS
    SQLLocalDBView Uninstaller

.DESCRIPTION
    Uninstalls the SQLLocalDBView Windows Explorer Shell Extension.
...
function Invoke-KillOnly {
    ...
    Write-Host "           SQLLocalDBView Process Killer" -ForegroundColor Yellow
    Write-Host "================================================================" -ForegroundColor Yellow
    Write-Host ""
...
function Invoke-Uninstall {
    ...
    Write-Host "           SQLLocalDBView Uninstaller" -ForegroundColor Cyan
    ...
    if (-not $dllExists -and -not $registryExists) {
        Write-Warn "SQLLocalDBView does not appear to be installed"
        return
    }
    ...
    try {
        ...
        foreach ($fileName in @("SQLLocalDBView.dll", "SQLLocalDBView.pdb", "SQLLocalDBView.exp", "SQLLocalDBView.lib")) {
