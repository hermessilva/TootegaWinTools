...
// Product information
#define PRODUCT_NAME        L"SQLiteView"
#define PRODUCT_VERSION     L"1.0.0"
#define PRODUCT_PUBLISHER   L"Tootega Pesquisa e Inovacao"
#define DLL_NAME            L"SQLiteView.dll"
#define INSTALL_SUBDIR      L"Tootega\\SQLiteView"

// Registry paths for uninstall information
#define UNINSTALL_KEY       L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\SQLiteView"
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
    ...
    if (doInstall) {
        ...
        std::wstring setupTarget = installDir + L"\\SQLiteViewSetup.exe";
...
bool RemoveInstallDir(const std::wstring& installDir)
{
    ...
    std::wstring pdbPath = installDir + L"\\SQLiteView.pdb";
    DeleteFileW(pdbPath.c_str());
    
    std::wstring setupPath = installDir + L"\\SQLiteViewSetup.exe";
...
int ShowInstallDialog()
{
    return MessageBoxW(nullptr,
        L"SQLiteView is not installed.\n\n"
        L"Do you want to install SQLiteView?\n\n"
        L"This will add support for viewing SQLite databases\n"
...
{
    return MessageBoxW(nullptr,
        L"SQLiteView is already installed.\n\n"
        L"Do you want to uninstall SQLiteView?\n\n"
...
void ShowSuccessMessage(bool isInstall)
{
    if (isInstall) {
        MessageBoxW(nullptr,
            L"SQLiteView has been installed successfully!\n\n"
            L"You can now browse SQLite databases directly in\n"
            L"Windows Explorer.\n\n"
            L"Right-click on a .db file to see additional options.",
            L"SQLiteView Setup",
            MB_OK | MB_ICONINFORMATION);
    }
    else {
        MessageBoxW(nullptr,
            L"SQLiteView has been uninstalled successfully.\n\n"
            L"Thank you for using SQLiteView!",
            L"SQLiteView Setup",
            MB_OK | MB_ICONINFORMATION);
    }
}
...
void ShowErrorMessage(const std::wstring& message)
{
    MessageBoxW(nullptr, message.c_str(), L"SQLiteView Setup - Error", MB_OK | MB_ICONERROR);
}
