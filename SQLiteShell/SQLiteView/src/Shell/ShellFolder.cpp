...
STDMETHODIMP ShellFolder::QueryInterface(REFIID riid, void** ppv) {
    ...
    if (IsEqualIID(riid, IID_IUnknown) ||
        ...
        *ppv = static_cast<IShellFolder2*>(this);
        ...
        SQLITEVIEW_LOG(L"  -> Returning IPersistFile");
...
STDMETHODIMP ShellFolder::Initialize(PCIDLIST_ABSOLUTE pidl) {
    ...
    // Extract database path from PIDL
...
STDMETHODIMP ShellFolder::EnumObjects(HWND hwnd, SHCONTF grfFlags, IEnumIDList** ppenumIDList) {
    ...
    *ppenumIDList = nullptr;
    ...
    SQLITEVIEW_LOG(L"EnumObjects: flags=0x%08X table='%s'", 
...
STDMETHODIMP ShellFolder::BindToObject(PCUIDLIST_RELATIVE pidl, IBindCtx* pbc, REFIID riid, void** ppv) {
    ...
    *ppv = nullptr;
    ...
    SQLITEVIEW_LOG(L"BindToObject: name=%s riid=%s", item->name, guidStr);
    ...
    // Initialize with combined PIDL
    if (_PidlRoot) {
        PIDLIST_ABSOLUTE newPidl = ILCombine(_PidlRoot, pidl);
        if (newPidl) {
            subFolder->_PidlRoot = newPidl;
        }
    }
...
STDMETHODIMP ShellFolder::CreateViewObject(HWND hwndOwner, REFIID riid, void** ppv) {
    ...
    *ppv = nullptr;
    ...
    wchar_t guidStr[64];
    StringFromGUID2(riid, guidStr, 64);
    SQLITEVIEW_LOG(L"CreateViewObject: riid=%s", guidStr);

    if (IsEqualIID(riid, IID_IShellView)) {
        // Create default shell view
        SFV_CREATE sfvc = {};
        sfvc.cbSize = sizeof(sfvc);
        sfvc.pshf = static_cast<IShellFolder*>(this);
        sfvc.psfvcb = static_cast<IShellFolderViewCB*>(this);
        
        return SHCreateShellFolderView(&sfvc, reinterpret_cast<IShellView**>(ppv));
    }
...
STDMETHODIMP ShellFolder::GetAttributesOf(UINT cidl, PCUITEMID_CHILD_ARRAY apidl, SFGAOF* rgfInOut) {
    ...
    for (UINT i = 0; i < cidl; i++) {
        ...
        if (item->type == ItemType::Table || 
            ...
            item->type == ItemType::SystemTable) {
            // Tables are folders
            attrs = SFGAO_FOLDER | SFGAO_BROWSABLE | SFGAO_HASSUBFOLDER;
        } else {
            // Records are files
...
STDMETHODIMP ShellFolder::GetUIObjectOf(HWND hwndOwner, UINT cidl, PCUITEMID_CHILD_ARRAY apidl,
    ...
    *ppv = nullptr;
    ...
    if (IsEqualIID(riid, IID_IContextMenu) || IsEqualIID(riid, IID_IContextMenu2) || IsEqualIID(riid, IID_IContextMenu3)) {
        ...
        // TODO: Initialize menu with selected items
    ...
    if (_CurrentTable.empty()) {
        ...
        switch (iColumn) {
            ...
            case 1: // RowID
                ...
                break;
            default: {
                // Get column value from record data
                // For this we need to query the actual record
                if (_Database && item->rowid >= 0) {
                    DatabaseEntry entry = _Database->GetRecordByRowID(_CurrentTable, item->rowid);
                    LoadColumns();
                    if (iColumn - 2 < _CurrentColumns.size()) {
...
void EnumIDList::Initialize() {
    ...
    if (!_Folder || !_Folder->OpenDatabase()) return;
    ...
    SQLITEVIEW_LOG(L"EnumIDList: enumerated %zu items", _Items.size());
}
