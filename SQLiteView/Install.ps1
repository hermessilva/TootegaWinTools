...
$TargetPdb = Join-Path $InstallDir "SQLiteView.pdb"
...
function Show-Header {
    ...
    Write-Host "              SQLiteView $Title" -ForegroundColor Cyan
...
function Register-Extension {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "      $env:TEMP\SQLiteView_Debug.log" -ForegroundColor Gray
        Write-Host ""
...
function Show-RegistrationStatus {
    ...
    $checks = @(
        ...
        @{ Name = "ProgId"; Key = "HKCU\Software\Classes\SQLiteView.Database" },
        @{ Name = ".db Extension"; Key = "HKCU\Software\Classes\.db" }
    )
...
function Install-SQLiteView {
    ...
    $sourcePdb = Join-Path $BuildDir "SQLiteView.pdb"
    ...
    try {
        ...
        if (Test-Path $sourcePdb) {
            ...
            Write-OK "Copied SQLiteView.pdb"
        }
...
function Uninstall-SQLiteView {
    ...
    if (-not (Test-Path $TargetDll)) {
        Write-Warn "SQLiteView is not installed at $InstallDir"
        return
    }
    ...
    try {
        ...
        $filesToRemove = @($TargetDll, $TargetPdb, (Join-Path $InstallDir "SQLiteView.exp"), (Join-Path $InstallDir "SQLiteView.lib"))
        $failedFiles = @()
