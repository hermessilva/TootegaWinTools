<#
.SYNOPSIS
    SQLiteView Debug Test Script

.DESCRIPTION
    Tests the SQLiteView extension and shows debug logs.
...
$LogFile = Join-Path $env:TEMP "SQLiteView_Debug.log"

Write-Host ""
Write-Host "╔════════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "║             SQLiteView Debug Test Script                       ║" -ForegroundColor Cyan
Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
$progid = reg query "HKCR\SQLiteView.Database" 2>&1
if ($LASTEXITCODE -eq 0) {
...
$ext = reg query "HKCR\.db" 2>&1
if ($ext -match "SQLiteView.Database") {
    Write-Host "  [OK] .db extension associated" -ForegroundColor Green
} else {
    Write-Host "  [WARN] .db extension not associated with SQLiteView.Database" -ForegroundColor Yellow
}

# Show shell command
Write-Host ""
Write-Host "Shell Open Command:" -ForegroundColor White
reg query "HKCR\SQLiteView.Database\shell\open\command" 2>&1 | ForEach-Object { Write-Host "  $_" -ForegroundColor Gray }
...
$testDb = Join-Path $env:TEMP "SQLiteView_Test.db"
Write-Host ""
...
# We can't easily create SQLite db from PowerShell without sqlite3.exe
...
Write-Host "  3. Find: SQLiteView_Test.db" -ForegroundColor Cyan
