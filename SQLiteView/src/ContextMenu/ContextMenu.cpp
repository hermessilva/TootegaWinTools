...
STDMETHODIMP ContextMenuHandler::SetSite(IUnknown* pUnkSite) {
    site_ = pUnkSite;
    SQLITEVIEW_LOG(L"ContextMenuHandler::SetSite called, site=%p", pUnkSite);
    return S_OK;
}
...
STDMETHODIMP ContextMenuHandler::Initialize(PCIDLIST_ABSOLUTE pidlFolder, 
    ...
    SQLITEVIEW_LOG(L"ContextMenuHandler::Initialize called");
        ...
        if (SUCCEEDED(pdtobj->GetData(&fmt, &stg))) {
            ...
            if (hDrop) {
                ...
                if (DragQueryFileW(hDrop, 0, path, MAX_PATH)) {
                    dbPath_ = path;
                    SQLITEVIEW_LOG(L"ContextMenu: File path = %s", path);
                }
    ...
    if (dbPath_.empty() && pidlFolder) {
        ...
        if (SHGetPathFromIDListW(pidlFolder, path)) {
            dbPath_ = path;
            SQLITEVIEW_LOG(L"ContextMenu: Folder path = %s", path);
        }
    }
...
STDMETHODIMP ContextMenuHandler::QueryContextMenu(HMENU hmenu, UINT indexMenu, 
    ...
    SQLITEVIEW_LOG(L"QueryContextMenu: uFlags=0x%X, idCmdFirst=%u, tableName='%s'", 
        uFlags, idCmdFirst, tableName_.c_str());
    
    cmdFirst_ = idCmdFirst;
    
    // Se CMF_DEFAULTONLY, fornecer apenas o comando "Open" para navegação
    if (uFlags & CMF_DEFAULTONLY) {
        // Inserir apenas o comando Open como padrão
        InsertMenuW(hmenu, indexMenu, MF_BYPOSITION | MF_STRING, 
            idCmdFirst + CMD_OPEN, L"&Open");
        
        // Marcar como item padrão (negrito)
        SetMenuDefaultItem(hmenu, indexMenu, TRUE);
        
        SQLITEVIEW_LOG(L"QueryContextMenu: CMF_DEFAULTONLY - Added Open command");
        return MAKE_HRESULT(SEVERITY_SUCCESS, 0, CMD_OPEN + 1);
    }

    // Menu completo
    // Primeiro, adicionar "Open" como item padrão
    InsertMenuW(hmenu, indexMenu, MF_BYPOSITION | MF_STRING, 
        idCmdFirst + CMD_OPEN, L"&Open");
    SetMenuDefaultItem(hmenu, indexMenu, TRUE);

    // Criar submenu SQLiteView
    ...
    mii.dwTypeData = (LPWSTR)L"SQLiteView";
    
    InsertMenuItemW(hmenu, indexMenu + 1, TRUE, &mii);  // +1 porque "Open" está em indexMenu
...
STDMETHODIMP ContextMenuHandler::InvokeCommand(CMINVOKECOMMANDINFO* pici) {
    ...
    if (HIWORD(pici->lpVerb)) {
        ...
        if (_stricmp(verb, "open") == 0) {
            ...
            SQLITEVIEW_LOG(L"InvokeCommand: Unknown verb '%S'", verb);
            return E_INVALIDARG;
        }
    } else {
    ...
    SQLITEVIEW_LOG(L"InvokeCommand: cmd=%u, itemType=%d, dbPath='%s', tableName='%s'", 
    ...
    if (cmd == CMD_OPEN && (itemType_ == ItemType::Table || itemType_ == ItemType::View)) {
        SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - Attempting navigation, pidlItem_=%p, site_=%p", 
            pidlItem_, site_);
        ...
        if (site_) {
            ...
            if (pShellBrowser && pidlItem_) {
                SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - Using IShellBrowser::BrowseObject with pidlItem_");
                ...
                if (SUCCEEDED(hr)) {
                    SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - BrowseObject succeeded");
                    pShellBrowser->Release();
                    return S_OK;
                }
                SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - BrowseObject failed: 0x%08X", hr);
                pShellBrowser->Release();
            }
        ...
        SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - Navigating to '%s'", virtualPath.c_str());
        ...
        if (SUCCEEDED(hr) && pidl) {
            ...
            if (site_) {
                ...
                if (pShellBrowser) {
                    ...
                    if (SUCCEEDED(hr)) {
                        SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - BrowseObject (absolute) succeeded");
                        pShellBrowser->Release();
                        CoTaskMemFree(pidl);
                        return S_OK;
                    }
                    SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - BrowseObject (absolute) failed: 0x%08X", hr);
                    pShellBrowser->Release();
                }
            }
            ...
            if (SUCCEEDED(hr)) {
                SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - SHOpenFolderAndSelectItems succeeded");
            } else {
                SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - SHOpenFolderAndSelectItems failed: 0x%08X", hr);
            }
            
            CoTaskMemFree(pidl);
            return hr;
        } else {
            SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - SHParseDisplayName failed: 0x%08X", hr);
            ...
            if ((INT_PTR)result > 32) {
                SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - ShellExecute succeeded");
                return S_OK;
            } else {
                SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN - ShellExecute failed: %d", (int)(INT_PTR)result);
            }
        }
    ...
    if (cmd == CMD_OPEN) {
        SQLITEVIEW_LOG(L"InvokeCommand: CMD_OPEN ignored for non-folder item type %d", (int)itemType_);
        return S_OK;
    }
    
    // Verificar se temos caminho para outros comandos
    if (dbPath_.empty()) {
        MessageBoxW(pici->hwnd, L"No database file selected", L"SQLiteView Error", MB_ICONERROR);
        return E_FAIL;
    }
    ...
    if (!database_ || !database_->IsOpen()) {
        ...
        MessageBoxW(pici->hwnd, msg.c_str(), L"SQLiteView Error", MB_ICONERROR);
        return E_FAIL;
    }
    ...
    try {
        switch (cmd) {
            ...
            case CMD_VIEW_INDEXES:
                ...
                break;
            default:
                MessageBoxW(pici->hwnd, L"Unknown command", L"SQLiteView", MB_ICONWARNING);
                return E_INVALIDARG;
        }
    }
        ...
        MessageBoxW(pici->hwnd, L"Unknown error occurred", L"SQLiteView Error", MB_ICONERROR);
        return E_FAIL;
    }
...
std::wstring ContextMenuHandler::GenerateInsertStatements(int maxRows) {
    ...
    for (const auto& row : result.rows) {
        oss << L"INSERT INTO \"" << tableName_ << L"\" (";
...
STDMETHODIMP DatabaseContextMenuHandler::Initialize(PCIDLIST_ABSOLUTE pidlFolder, 
    ...
    // MessageBoxW(nullptr, L"DatabaseContextMenuHandler::Initialize called!", L"SQLiteView DEBUG", MB_OK | MB_ICONINFORMATION);
    
    SQLITEVIEW_LOG(L"DatabaseContextMenuHandler::Initialize called");
    OutputDebugStringW(L"[SQLiteView] DatabaseContextMenuHandler::Initialize called\n");
    selectedFiles_.clear();
    
    if (!pdtobj) {
        SQLITEVIEW_LOG(L"DatabaseContextMenuHandler: pdtobj is null");
        return E_INVALIDARG;
    }
    ...
    if (FAILED(hr)) {
        SQLITEVIEW_LOG(L"DatabaseContextMenuHandler: GetData failed hr=0x%08X", hr);
        return hr;
    }
    ...
    if (hDrop) {
        ...
        SQLITEVIEW_LOG(L"DatabaseContextMenuHandler: Found %u files", count);
        for (UINT i = 0; i < count; i++) {
            wchar_t path[MAX_PATH];
            if (DragQueryFileW(hDrop, i, path, MAX_PATH)) {
                selectedFiles_.push_back(path);
                SQLITEVIEW_LOG(L"DatabaseContextMenuHandler: File[%u] = %s", i, path);
            }
        }
        GlobalUnlock(stg.hGlobal);
    }
...
STDMETHODIMP DatabaseContextMenuHandler::QueryContextMenu(HMENU hmenu, UINT indexMenu, 
    ...
    InsertMenuW(hmenu, indexMenu++, MF_BYPOSITION | MF_STRING, 
        idCmdFirst + 0, L"Browse with SQLiteView");
    InsertMenuW(hmenu, indexMenu++, MF_BYPOSITION | MF_STRING, 
...
STDMETHODIMP DatabaseContextMenuHandler::InvokeCommand(CMINVOKECOMMANDINFO* pici) {
    ...
    SQLITEVIEW_LOG(L"DatabaseContextMenuHandler::InvokeCommand called");
    
    if (selectedFiles_.empty()) {
        MessageBoxW(pici->hwnd, L"No file selected", L"SQLiteView Error", MB_ICONERROR);
        return E_FAIL;
    }
    ...
    SQLITEVIEW_LOG(L"DatabaseContextMenuHandler: cmd=%u, path=%s", cmd, path.c_str());
...
void DatabaseContextMenuHandler::ExportAllTables(HWND hwnd, const std::wstring& path) {
    ...
    if (!db || !db->IsOpen()) {
        MessageBoxW(hwnd, L"Failed to open database", L"SQLiteView", MB_ICONERROR);
        return;
    }
    ...
    MessageBoxW(hwnd, msg.c_str(), L"SQLiteView", MB_ICONINFORMATION);
}
...
void DatabaseContextMenuHandler::ShowDatabaseInfo(HWND hwnd, const std::wstring& path) {
    ...
    std::wostringstream oss;
    oss << L"Database: " << info.fileName << L"\n\n";
    oss << L"File Size: " << (info.fileSize / 1024) << L" KB\n";
    oss << L"Page Size: " << info.pageSize << L" bytes\n";
    oss << L"Page Count: " << info.pageCount << L"\n";
    oss << L"Encoding: " << info.encoding << L"\n";
    oss << L"Schema Version: " << info.schemaVersion << L"\n";
    oss << L"User Version: " << info.userVersion << L"\n";
    oss << L"Journal Mode: " << (info.isWAL ? L"WAL" : L"DELETE") << L"\n\n";
...
void DatabaseContextMenuHandler::ValidateDatabase(HWND hwnd, const std::wstring& path) {
    ...
    QueryResult result = db->ExecuteQuery(L"PRAGMA integrity_check");
    
    if (result.error.empty() && !result.rows.empty() && !result.rows[0].empty()) {
        std::wstring status = result.rows[0][0].ToString();
        ...
        MessageBoxW(hwnd, L"Failed to run integrity check", L"Error", MB_ICONERROR);
    }
}
