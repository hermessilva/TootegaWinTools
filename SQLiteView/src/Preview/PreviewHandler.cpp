...
const wchar_t* PreviewHandler::WINDOW_CLASS = L"SQLiteViewPreview";
...
STDMETHODIMP PreviewHandler::Initialize(LPCWSTR pszFilePath, DWORD grfMode) {
    ...
    SQLITEVIEW_LOG(L"PreviewHandler::Initialize file=%s table=%s", 
...
STDMETHODIMP PreviewHandler::Initialize(IStream* pstream, DWORD grfMode) {
    // SQLite não suporta streams diretamente, precisamos de arquivo físico
    return E_NOTIMPL;
}
...
bool PreviewHandler::CreatePreviewWindow() {
    ...
    if (!hwndPreview_) {
        SQLITEVIEW_LOG(L"Failed to create preview window: %d", GetLastError());
        return false;
    }
...
bool PreviewHandler::LoadTablesList() {
    ...
    SQLITEVIEW_LOG(L"LoadTablesList: Found %d tables/views", (int)tablesList_.size());
...
void PreviewHandler::RenderTablesList(HDC hdc, const RECT& rc) {
    ...
    std::wstring title = L"SQLite Database: " + dbName + L" (" + 
    ...
    for (const auto& table : tablesList_) {
        ...
        if (y + config_.rowHeight > colHeaderRect.bottom) {
            ...
            std::wstring colsStr = std::to_wstring(table.columns.size());
