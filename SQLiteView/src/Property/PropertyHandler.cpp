...
static const PROPERTYKEY PKEY_SQLite_TableCount = { { 0xD9FFDCA0, 0x1234, 0x5678, { 0x9A, 0xBC, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF } }, 1 };
static const PROPERTYKEY PKEY_SQLite_ViewCount = { { 0xD9FFDCA0, 0x1234, 0x5678, { 0x9A, 0xBC, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF } }, 2 };
static const PROPERTYKEY PKEY_SQLite_IndexCount = { { 0xD9FFDCA0, 0x1234, 0x5678, { 0x9A, 0xBC, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF } }, 3 };
static const PROPERTYKEY PKEY_SQLite_PageSize = { { 0xD9FFDCA0, 0x1234, 0x5678, { 0x9A, 0xBC, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF } }, 4 };
static const PROPERTYKEY PKEY_SQLite_Encoding = { { 0xD9FFDCA0, 0x1234, 0x5678, { 0x9A, 0xBC, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF } }, 5 };
...
STDMETHODIMP PropertyHandler::Initialize(IStream* pstream, DWORD grfMode) {
    // SQLite não suporta streams diretamente
    return E_NOTIMPL;
}
...
void PropertyHandler::LoadProperties() {
    ...
    AddProperty(PKEY_ItemType, L"SQLite Database");
    AddProperty(PKEY_ItemTypeText, L"SQLite Database File");
    AddProperty(PKEY_FileDescription, L"SQLite Database");
    ...
    // Encoding
    AddProperty(PKEY_SQLite_Encoding, dbInfo_.encoding);
    
    // Page size
    AddProperty(PKEY_SQLite_PageSize, (ULONG)dbInfo_.pageSize);
    
    // Counts
    AddProperty(PKEY_SQLite_TableCount, (ULONG)dbInfo_.tables.size());
    AddProperty(PKEY_SQLite_ViewCount, (ULONG)dbInfo_.views.size());
    AddProperty(PKEY_SQLite_IndexCount, (ULONG)dbInfo_.indexes.size());
}
